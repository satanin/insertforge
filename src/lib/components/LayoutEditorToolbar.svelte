<script lang="ts">
  ...
  interface Props {
    ...
    selectedDims: { width: number; depth: number } | null;
    itemTypeLabel?: string; // Optional label like "Box" or "Tray"
    canEdit?: boolean;
    onEnterEdit: () => void;
    onSave: () => void;
    onCancel: () => void;
...
{:else}
  ...
  {#if selectedItem && selectedDims}
    <div class="selectionInfo">
      <span class="selectionName">{selectedItem.name}</span>
      {#if itemTypeLabel}
        <span class="selectionType">{itemTypeLabel}</span>
      {/if}
      <span class="selectionDims">
        {selectedDims.width.toFixed(0)} × {selectedDims.depth.toFixed(0)}mm
      </span>
      <span class="selectionRotation">{selectedItem.rotation}°</span>
