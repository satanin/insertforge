...
export function updateSplitRatio(layout: CupLayout, cupId: CupId, newRatio: number): CupLayout {
	...
	function findAndUpdateSplit(node: CupLayoutNode): CupLayoutNode {
		...
		if (firstContainsCup || secondContainsCup) {
			...
			if (
				(isCupLeaf(node.first) && node.first.id === cupId) ||
				(isCupLeaf(node.second) && node.second.id === cupId)
			) {
...
export function updateParentSplitRatio(
	layout: CupLayout,
	cupId: CupId,
	newRatio: number
): CupLayout {
...
export function updateSplitRatioByPath(
	layout: CupLayout,
	path: string,
	newRatio: number
): CupLayout {
